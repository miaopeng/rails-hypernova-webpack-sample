...
/***/ function(module, exports, __webpack_require__) {
	...
	var renderReact = __webpack_require__(37).renderReact;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _hypernova = __webpack_require__(172);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ReactMarkupChecksum = __webpack_require__(163);
	var ReactReconciler = __webpack_require__(62);
	var ReactServerBatchingStrategy = __webpack_require__(171);
	var ReactServerRenderingTransaction = __webpack_require__(128);
	var ReactUpdates = __webpack_require__(59);
...
/***/ function(module, exports) {
	...
	function encode(obj) {
	  return ENCODE.reduce(function (str, coding) {
	    ...
	    var encodeChar = _coding[0];
	    var htmlEntity = _coding[1];
	...
	function decode(res) {
	  var jsonPayload = ENCODE.reverse().reduce(function (str, coding) {
	    var _coding2 = _slicedToArray(coding, 2);

	    var encodeChar = _coding2[0];
	    var htmlEntity = _coding2[1];
	...
	function backwardCompat(node, attr, key) {
	  return String(node) + '[data-mystique-key="' + String(key) + '"],' + String(node) + '[data-' + String(attr) + '="' + String(key) + '"]';
	}

	function toScript(attr, key, props) {
	  return '<script type="application/json" data-' + String(attr) + '="' + String(key) + '">' + LEFT + String(encode(props)) + RIGHT + '</script>'; // eslint-disable-line max-len
	}

	function fromScript(attr, key) {
	  var node = document.querySelector(backwardCompat('script', attr, key));
	  if (!node) return null;
	...
	function serialize(name, html, data) {
	  ...
	  var markup = '<div data-hypernova-key="' + String(key) + '">' + String(html) + '</div>';
	  var script = toScript('hypernova-key', key, data);
	  return markup + '\n' + String(script);
	}

	function load(name) {
	  var key = name.replace(/\W/g, '');
	  var node = document.querySelector(backwardCompat('div', 'hypernova-key', key));
	  if (!node) return {};

	  var data = fromScript('hypernova-key', key);
